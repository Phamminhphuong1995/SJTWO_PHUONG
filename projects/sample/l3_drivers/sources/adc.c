...
uint16_t adc__get_burst_mode_value(adc_channel_e channel_num) {
  uint16_t result;
  ...
  if ((ADC__CHANNEL_2 == channel_num) || (ADC__CHANNEL_4 == channel_num) || (ADC__CHANNEL_5 == channel_num)) {
    // START needs to be cleared as well as channel bits
    LPC_ADC->CR &= ~(channel_masks | start_conversion_mask);
    ...
    read_channel = (LPC_ADC->GDR >> 24) & 0x07;
    // fprintf(stderr, "Reading from channel %u \n", read_channel);
    result = (LPC_ADC->GDR >> 4) & twelve_bits; // 12bits - B15:B4
  }
